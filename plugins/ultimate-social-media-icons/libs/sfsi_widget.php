		...
		function sfsi_prepairIcons($icon_name, $is_front = 0)
		{
			...
			switch ($icon_name) {
				...
				case "fbmessenger":
					...
					//print_r($sfsi_section2_options);die;
			...
			if (isset($icon) && !empty($icon) && filter_var($icon, FILTER_VALIDATE_URL)) {
				...
				$icons .= "<a class='" . $class . " sficn' data-effect='" . $mouse_hover_effect . "' $new_window  href='" . $url . "' " . (('vk' !== $icon_name) ? "id='sfsiid_" . $icon_name . "_icon'" : '') . " style='opacity:" . $icon_opacity . "' " . (isset($sfsi_onclick) ? 'onclick="' . $sfsi_onclick . '"' : '') . " >";
